...
// Strict OEM 3-Mode specifications per brand (NO Race mode!)
const OEM_MODELS_DATA = {
  "G3 Max": {
    brand: "Segway-Ninebot",
    modes: ["ECO", "DRIVE", "SPORT"],
    defaultSpeeds: [15, 25, 45],
    specKmh: "max. 55-70 km/h (Straße) • 127 km/h (Luft)",
    specMph: "max 34-43 mph (road) • 79 mph (air)",
    hasBoost: true
  },
  "Navee ST3 Pro": {
    brand: "Navee",
    modes: ["WALK", "DRIVE", "SPORT"],
    defaultSpeeds: [6, 20, 45],
    specKmh: "max. 45-60 km/h (Straße) • 95 km/h (Luft)",
    specMph: "max 28-37 mph (road) • 59 mph (air)",
    hasBoost: false
  },
  "E3 Pro": {
    brand: "Xiaomi",
    modes: ["PEDESTRIAN", "STANDARD", "SPORT"],
    defaultSpeeds: [6, 20, 40],
    specKmh: "max. 40-50 km/h (Straße) • 75 km/h (Luft)",
    specMph: "max 25-31 mph (road) • 47 mph (air)",
    hasBoost: false
  }
};

const DEFAULT_NEUTRAL_MODES = ["ECO", "DRIVE", "SPORT"];
...
const RANDOM_DEVICES_POOL = [
  { id: "iPhone 15 Pro", baseSignal: -52 },
  { id: "Galaxy S24 Ultra", baseSignal: -55 },
  { id: "Samsung TV QN90", baseSignal: -64 },
  { id: "LG OLED evo C3", baseSignal: -68 },
  { id: "Pixel 8a", baseSignal: -59 },
  { id: "Sony BRAVIA 4K", baseSignal: -73 }
];
...
const appState = {
  ...
  speedUnit: localStorage.getItem("tp_speed_unit") || "kmh", // "kmh" or "mph"
  activeTab: "dashboard",
  ...
  activeModeIndex: parseInt(localStorage.getItem("tp_active_mode_idx") || "1", 10),
...
// ----------------------------------------------------
// SPEED UNIT CONVERSIONS & RENDERING (KM/H vs MPH)
// ----------------------------------------------------
function setSpeedUnit(unit) {
  appState.speedUnit = unit;
  localStorage.setItem("tp_speed_unit", unit);

  document.getElementById("unitBtnKmh").classList.toggle("active", unit === "kmh");
  document.getElementById("unitBtnMph").classList.toggle("active", unit === "mph");
  document.getElementById("gaugeUnitText").textContent = unit.toUpperCase();

  const isDe = (appState.lang === "de");
  const airLimitChip = document.getElementById("gaugeChipTarget");
  if (unit === "mph") {
    airLimitChip.textContent = isDe ? "LUFT: 79 MPH" : "AIR: 79 MPH";
  } else {
    airLimitChip.textContent = isDe ? "LUFT: 127 KM/H" : "AIR: 127 KM/H";
  }

  updateSingleModelBenchmark();
  renderTuningModeSliders();
}

// ----------------------------------------------------
// DYNAMIC OEM 3-RIDING-MODES (DASHBOARD & TUNING)
// ----------------------------------------------------
function getCurrentModelConfig() {
  if (appState.connectedMeta && appState.connectedMeta.guess) {
    return OEM_MODELS_DATA[appState.connectedMeta.guess] || null;
  }
  return null;
}

function renderDashboardModes() {
  const container = document.getElementById("dashModesContainer");
  const badge = document.getElementById("dashModelModeBadge");
  const config = getCurrentModelConfig();

  const modes = config ? config.modes : DEFAULT_NEUTRAL_MODES;
  badge.textContent = config ? `${config.brand.toUpperCase()} (3-MODE)` : "NEUTRAL (3-MODE)";

  container.innerHTML = "";
  modes.forEach((modeName, idx) => {
    const btn = document.createElement("button");
    btn.className = `mode-tab ${idx === appState.activeModeIndex ? "active" : ""}`;
    btn.textContent = modeName;
    btn.onclick = () => {
      appState.activeModeIndex = idx;
      localStorage.setItem("tp_active_mode_idx", idx);
      renderDashboardModes();
      showToast(`Modus: ${modeName}`);
    };
    container.appendChild(btn);
  });
}

function renderTuningModeSliders() {
  const container = document.getElementById("tuningModesSlidersContainer");
  if (!container) return;
  const config = getCurrentModelConfig();
  const modes = config ? config.modes : DEFAULT_NEUTRAL_MODES;
  const defaultVals = config ? config.defaultSpeeds : [15, 25, 45];
  const unit = appState.speedUnit.toUpperCase();

  container.innerHTML = "";
  modes.forEach((modeName, idx) => {
    let speedVal = defaultVals[idx];
    if (appState.speedUnit === "mph") speedVal = Math.round(speedVal * 0.621371);

    const row = document.createElement("div");
    row.className = "mode-tuning-row";
    row.innerHTML = `
      <span class="mode-tag m${idx}">${modeName}</span>
      <input type="range" min="${idx === 0 ? 3 : 10}" max="${idx === 2 ? (appState.speedUnit === "mph" ? 79 : 127) : 45}" value="${speedVal}" class="pro-slider" data-mode-slider="${idx}">
      <span class="mode-metric" data-mode-metric="${idx}">${speedVal} ${unit}</span>
    `;

    const slider = row.querySelector(`[data-mode-slider="${idx}"]`);
    const metric = row.querySelector(`[data-mode-metric="${idx}"]`);
    slider.oninput = (e) => {
      metric.textContent = `${e.target.value} ${unit}`;
    };
...
function updateSingleModelBenchmark() {
  ...
  const config = OEM_MODELS_DATA[modelKey];
  const specText = config
    ? (appState.speedUnit === "mph" ? config.specMph : config.specKmh)
    : "max. 45-60 km/h (Straße)";
...
// ----------------------------------------------------
// BLE SCANNING: TIMED DISCOVERY + MUTED OTHER DEVICES
// ----------------------------------------------------
function startBleScanningSequence() {
  ...
  function revealNextDevice() {
    ...
    const card = document.createElement("div");
    card.className = `device-row-card ${dev.isScooter ? '' : 'other-device'} ${isConnected ? "connected-active" : ""}`;
    ...
    if (dev.isScooter) {
      ...
      // Clean muted gray card: No serial, no BLE/DRV version line!
      card.innerHTML = `
        <div class="dev-info">
          <span class="dev-guess-label">${guessLabel}</span>
          <span class="dev-title">${dev.bleName}</span>
...
function probeOtherDevice(deviceName) {
  ...
  desc.textContent = i18n("connect.probingOtherDevice", "Prüfe Geräte-UUID...");
  ...
  setTimeout(() => {
    ...
    showToast(i18n("connect.foreignDeviceNotice", "Verbindung abgewiesen: Dies ist ein fremdes Gerät (Smartphone/TV) und kein kompatibler E-Scooter!"));
  }, 2000);
}
...
function initEventHandlers() {
  ...
  // SPEED UNIT TOGGLE
  document.getElementById("unitBtnKmh").onclick = () => setSpeedUnit("kmh");
  document.getElementById("unitBtnMph").onclick = () => setSpeedUnit("mph");
  ...
  // QUICK CONTROLS: Protected when disconnected!
...
async function startApplication() {
  ...
  setSpeedUnit(appState.speedUnit);
  updateHeaderStatus();
  updateSingleModelBenchmark();
  renderDashboardModes();
  renderTuningModeSliders();
