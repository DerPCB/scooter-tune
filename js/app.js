...
function updateSpeedCalculations() {
  ...
  const weight = appState.riderWeight;
  const isDe = (appState.lang === "de");

  document.getElementById("valSpeedLimit").textContent = `${set} km/h (in der Luft)`;
  document.getElementById("valRealSpeed").textContent = isDe ? "DEUTLICH WENIGER" : "SIGNIFICANTLY LOWER";
  document.getElementById("gaugeChipTarget").textContent = `LUFT: ${set} KM/H`;
  document.getElementById("gaugeChipEffective").textContent = isDe ? "STRASSE: VIEL WENIGER" : "ROAD: MUCH LOWER";

  const penaltyNote = document.getElementById("valWeightPenaltyNote");
  if (set > 25) {
    penaltyNote.textContent = isDe
      ? `Achtung: ${set} km/h dreht der Motor NUR FREI IN DER LUFT! Auf Asphalt mit ${weight} kg Fahrergewicht fährt er DEUTLICH WENIGER und weicht extrem ab.`
      : `Notice: ${set} km/h only spins FREE IN THE AIR! On asphalt with ${weight} kg rider mass, effective road speed is MUCH LOWER and deviates heavily.`;
  } else {
    penaltyNote.textContent = isDe
      ? `Reale Geschwindigkeit auf der Straße weicht durch ${weight} kg Fahrergewicht und Rollwiderstand nach unten ab.`
      : `Real road speed deviates downward due to ${weight} kg rider mass and rolling resistance.`;
  }
}
...
function initEventHandlers() {
  ...
  document.getElementById("protocolBLE").onclick = (e) => {
  ...
  document.getElementById("protocolWired").onclick = (e) => {
  ...
  const inputWeight = document.getElementById("inputRiderWeight");
  inputWeight.value = appState.riderWeight;
  inputWeight.oninput = (e) => {
    appState.riderWeight = parseInt(e.target.value, 10) || 75;
    localStorage.setItem("tp_rider_weight", appState.riderWeight);
    updateSpeedCalculations();
  };
