...
// Realistic OEM BLE Broadcast identifiers modeled after Ninebot / Segway / Xiaomi ESC nomenclature
...
const appState = {
  ...
  recoveryPin: localStorage.getItem("tp_recovery_pin") || "739214",
...
function updatePanicHintText() {
  ...
  const tmpl = i18n("tuning.stealthHint", "Combination: {n} / 12 selected (min. 2 required)");
...
function initEventHandlers() {
  ...
  document.getElementById("btnSavePanic").onclick = () => {
    if (appState.panicChips.length < 2) {
      showToast((appState.lang === "de") ? "Wähle mindestens 2 Tasten" : "Select at least 2 triggers");
      return;
    }
    const banner = document.getElementById("stealthStatusBanner");
    banner.classList.remove("hidden");
    showToast(i18n("tuning.stealthActive", "Police mode: STEALTH ARMED"));
  };
