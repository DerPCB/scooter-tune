...
// ----------------------------------------------------
// BLE SCANNING: TIMED DISCOVERY + CLEAN MUTED OTHER DEVICES
// ----------------------------------------------------
function startBleScanningSequence() {
  ...
  function revealNextDevice() {
    ...
    if (dev.isScooter) {
      // Full detailed scooter card with serial & DRV & Connect button
      card.innerHTML = `
        <div class="dev-info">
      ...
      // CLEAN MUTED CARD FOR PHONES / TVS: No serial, no BLE/DRV version line!
      card.innerHTML = `
        <div class="dev-info">
          <span class="dev-guess-label">${guessLabel}</span>
          <span class="dev-title">${dev.bleName}</span>
      ...
      // Clicking on other device shows modal probe with 2s delay then rejection toast!
      card.onclick = () => {
